...
def train(opt):
    ...
    game_state = Game()
    image, reward, terminal, _ = game_state.nextFrame(0)
    image = pre_processing(image[:game_state.win.get_width(), :int(game_state.win.get_height() * 0.79)], 84, 84)
    ...
    while iter < opt.num_iters:
        ...
        next_image, reward, terminal, _ = game_state.nextFrame(action)
        if terminal:
            print(terminal)
            game_state = Game()
            image, reward, terminal, _ = game_state.nextFrame(0)
            ...
            if torch.cuda.is_available():
                ...
                image = image.cuda()
            state = torch.cat(tuple(image for _ in range(4)))[None, :, :, :]
        next_image = pre_processing(next_image[:game_state.win.get_width(), :int(game_state.win.get_height() * 0.79)], opt.image_size,
                                    opt.image_size)
        next_image = torch.from_numpy(next_image)
        if torch.cuda.is_available():
            next_image = next_image.cuda()
        next_state = state = torch.cat(tuple(image for _ in range(4)))[None, :, :, :]
